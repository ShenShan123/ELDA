...
def networkx_cache(fn):
    cache = PriorityDictionary(1024)

    def wrapped_fn(*args, **kwargs):
        fingeprint = joblib.hash((args, kwargs))
        if fingeprint in cache:
            return cache[fingeprint]
        result = fn(*args, **kwargs)
        cache[fingeprint] = result
        return result
...
def orca(graph):
    if graph.number_of_nodes() == 0:
        return np.zeros((0, 15), dtype=int)
    ...
    if not parsed:
        # Fallback to zeros if ORCA output is empty/malformed.
        node_orbit_counts = np.zeros((graph.number_of_nodes(), 15), dtype=int)
    else:
        node_orbit_counts = np.array(parsed, dtype=int)
        # If ORCA outputs fewer lines than nodes, pad with zeros.
        if node_orbit_counts.shape[0] < graph.number_of_nodes():
            pad = np.zeros((graph.number_of_nodes() - node_orbit_counts.shape[0], node_orbit_counts.shape[1]), dtype=int)
            node_orbit_counts = np.vstack([node_orbit_counts, pad])
