...
1. four epochs from scratch with the ELDA compact-load representation;
...
- ELDA four-epoch scratch training;
- ELDA checkpoint selection and continuation;
...
hashes are retained in the source repository. The recorded checkpoint lineage
must use the pre-audit lists. New training defaults to the strict audited view
unless the lineage reproduction preset is selected explicitly.

## Frozen paper evidence

The full artifact bundle contains the final ELDA evidence needed by the paper:
...
Superseded experiments and duplicate checkpoints are outside the publication
scope.
